 ...
 *
 * THIS FILE IS AVAILABLE UNDER TWO DIFFERENT LICENSES: 1/GPL OR 2/JSEL
 *
 * 1/ GPL ==================================================================================
 *
 ...
 *
 * Alternatively, commercial and supported versions of the program - also known as Enterprise
 * Distributions - must be used in accordance with the terms and conditions contained in a separate
...
public class WebprojectHandler implements Serializable {
  ...
  private static final HashSet<String> NON_SITE_IMPORTS =
      new HashSet<>(Arrays.asList("serverPermissions.xml", "users.xml", "users.zip",
          JahiaSitesService.SYSTEM_SITE_KEY + ".zip", "references.zip", "roles.zip", "mounts.zip"));
  ...
  public WebprojectHandler() {
    ...
    File[] files = new File(SettingsBean.getInstance().getJahiaVarDiskPath() + "/prepackagedSites")
        .listFiles();
    if (files != null) {
      for (File file : files) {
    ...
    for (final JahiaTemplatesPackage aPackage : ServicesRegistry.getInstance()
        .getJahiaTemplateManagerService().getAvailableTemplatePackages()) {
      final Bundle bundle = aPackage.getBundle();
      if (bundle != null) {
        final Enumeration<URL> resourceEnum =
            bundle.findEntries("META-INF/prepackagedSites/", "*", false);
        ...
        while (resourceEnum.hasMoreElements()) {
          ...
          try {
            ...
            try {
              String titleKey = "serverSettings.manageWebProjects.importprepackaged."
                  + bundleResource.getFilename();
              title = Messages.get(aPackage, titleKey, LocaleContextHolder.getLocale(),
                  bundleResource.getFilename());
            } catch (MissingResourceException e) {
              logger.warn("unable to get resource key "
                  + "serverSettings.manageWebProjects.importprepackaged."
                  + bundleResource.getFilename() + " in package" + bundle.getSymbolicName());
            }
            prepackagedSites
                .put(bundleResource.getURI().toString() + "#" + bundle.getSymbolicName(), title);
          } catch (IOException e) {
    ...
    try {
      template.doExecuteWithSystemSession(new JCRCallback<Object>() {
        ...
        public Object doInJCR(JCRSessionWrapper session) throws RepositoryException {
          try {
            JahiaSite site = sitesService.addSite(JCRSessionFactory.getInstance().getCurrentUser(),
                bean.getTitle(), bean.getServerName(), bean.getSiteKey(), bean.getDescription(),
                LanguageCodeConverters.getLocaleFromCode(bean.getLanguage()), bean.getTemplateSet(),
                bean.getModules().toArray(new String[bean.getModules().size()]), null, null, null,
            ...
            if (bean.isCreateAdmin()) {
              ...
              JCRUserNode adminSiteUser = userManagerService.createUser(admin.getUsername(),
                  admin.getPassword(), admin.getUserProperties(), session);
  ...
  private Locale determineDefaultLocale(Locale defaultLocale, ImportInfo infos) {
    ...
    if (!activeLanguageCodesByRank.isEmpty()) {
      defaultLocale = LanguageCodeConverters.languageCodeToLocale(
          activeLanguageCodesByRank.get(activeLanguageCodesByRank.firstKey()));
    }
  ...
  public void exportToFile(RequestContext requestContext, boolean staging) throws Exception {
    ...
    if (sitekeys != null) {
      for (String sitekey : sitekeys) {
        JahiaSite site =
            ServicesRegistry.getInstance().getJahiaSitesService().getSiteByKey(sitekey);
      ...
      while ((z = zis.getNextEntry()) != null) {
        ...
        if (!emptyFiles.isEmpty()) {
          ...
          messageContext.addMessage(new MessageBuilder().error()
              .code("serverSettings.manageWebProjects.import.emptyFiles").arg(emptyFiles).build());
          return;
        }
  ...
  public static Map<String, Resource> getLegacyMappingsInModules(final String pattern) {
    ...
    if (defaultMappingsFolder != null && defaultMappingsFolder.exists()) {
      try {
        Collection<File> filesList =
            FileUtils.listFiles(defaultMappingsFolder, new String[] {pattern}, false);
        if (filesList != null) {
    ...
    for (final JahiaTemplatesPackage aPackage : ServicesRegistry.getInstance()
      ...
      if (bundle != null) {
        final Enumeration<URL> resourceEnum =
            bundle.findEntries("META-INF/legacyMappings", "*." + pattern, false);
        if (resourceEnum == null)
          continue;
        while (resourceEnum.hasMoreElements()) {
          BundleResource bundleResource = new BundleResource(resourceEnum.nextElement(), bundle);
        ...
        try {
          BundleResource resource = new BundleResource(new URL(bundleInfos[0]),
              ServicesRegistry.getInstance().getJahiaTemplateManagerService()
                  .getTemplatePackageById(bundleInfos[1]).getBundle());
  ...
  private ImportInfo prepareSiteImport(File i, String filename, MessageContext messageContext)
      throws IOException {
          ...
          if ("site.properties".equals(name)) {
            ...
            if (p.containsKey("templatePackageName")) {
              JahiaTemplatesPackage pack = templateManagerService
                  .getTemplatePackageById((String) p.get("templatePackageName"));
              if (pack == null) {
                pack = templateManagerService
                    .getTemplatePackage((String) p.get("templatePackageName"));
              }
              if (pack != null) {
            ...
            isLegacySite = true;
          } else if (validityCheckOnImport && !isLegacyImport && name.contains("repository.xml")
              && !name.contains("/")) {
            try {
              ...
              final String[] messageParams =
                  {filename, name, String.valueOf((System.currentTimeMillis() - timer)),
                      validationResults.toString()};
              final boolean hasValidationErrors = !validationResults.isSuccessful();
              if (hasValidationErrors) {
                logger.error("Failed validation {}/{} validated in {} ms: {}", messageParams);
              } else {
              ...
              if (hasValidationErrors) {
                if (importInfos.getValidationResult() != null) {
                  ...
                  importInfos.setValidationResult(
                      importInfos.getValidationResult().merge(validationResults));
                } else {
  ...
  public boolean processImport(final JahiaUser user, MessageContext context) {
    return processInternalImport(user, context);
  }
  
  public void processAsyncImport(RequestContext flowRequestContext, final JahiaUser jahiaUser, MessageContext context) {
    // TODO do Async
    try {
  ...
  public void updateSite(SiteBean bean, MessageContext messages) {
    try {
      ...
      if (serverNameChanged || !StringUtils.equals(site.getTitle(), bean.getTitle())
      ...
      if (serverNameChanged) {
        ...
        for (String moduleId : site.getAllInstalledModules()) {
          JahiaTemplatesPackage templatePackage =
              jahiaTemplateManagerService.getTemplatePackageById(moduleId);
          CacheHelper.flushOutputCachesForPath(
              "/modules/" + templatePackage.getIdWithVersion() + "/templates", true);
        }
      }
      ...
      messages.addMessage(new MessageBuilder().error()
          .code("serverSettings.manageWebProjects.webProject.error").arg(e.getMessage()).build());
      logger.error(e.getMessage(), e);
    }
  }
  ...
  public String getDefaultTemplateSetId() {
    String id = null;
    String defTemplateSet =
        StringUtils.defaultIfBlank(SettingsBean.getInstance().lookupString("default_templates_set"),
  ...
  private void performAsynSiteImport(final JahiaUser currentUser, final ImportInfo infos,
      final Locale selectedLocale, final String selectTmplSet, final String[] modulesToDeploy,
      final String firstImport, final Resource fileImportResource,
      final Boolean doImportServerPermissions, final Resource legacyMappingFilePath,
      final Resource legacyDefinitionsFilePath) {
    logger.info("### [AFAC] --|-- Going to performe asynchronous import !!!");
    ExecutorService executorSrv = Executors.newSingleThreadScheduledExecutor();
    ListenableFuture<Boolean> importTask =
        MoreExecutors.listeningDecorator(executorSrv).submit(new Callable<Boolean>() {

          @Override
          public Boolean call() throws Exception {
            try {
              sitesService.addSite(currentUser, infos.getSiteTitle(), infos.getSiteServername(),
                  infos.getSiteKey(), "", selectedLocale, selectTmplSet, null, firstImport,
                  fileImportResource, infos.getImportFileName(), false, doImportServerPermissions,
                  infos.getOriginatingJahiaRelease(), legacyMappingFilePath,
    ...
    String processKey = "import_jahia_site_" + infos.getSiteKey() + "_" + currentUser.getUsername();
    AsyncImportSiteProcessManager.addSiteImportTask(processKey, importTask);
  }
  
  private boolean processInternalImport(final JahiaUser jahiaUser, final MessageContext context) {
          ...
          if (type.equals("files")) {
            try {
              final File file = ImportUpdateService.getInstance().updateImport(
                  infos.getImportFile(), infos.getImportFileName(), infos.getType(),
              ...
              final Map<String, String> pathMapping =
                  JCRSessionFactory.getInstance().getCurrentUserSession().getPathMapping();
              ...
              for (final ImportInfo infos2 : importsInfos.values()) {
                if (infos2.getOldSiteKey() != null && infos2.getSiteKey() != null
                    && !infos2.getOldSiteKey().equals(infos2.getSiteKey())) {
                  pathMapping.put("/sites/" + infos2.getOldSiteKey(),
                      "/sites/" + infos2.getSiteKey());
                }
              }
              JCRTemplate.getInstance().doExecuteWithSystemSessionAsUser(jahiaUser, null, null,
                  new JCRCallback<Object>() {
                @Override
                public Object doInJCR(JCRSessionWrapper session) throws RepositoryException {
                  try {
                    session.getPathMapping().putAll(pathMapping);
                    importExportBaseService.importSiteZip(
                        file == null ? null : new FileSystemResource(file), system,
                            infos.asMap(), null, null, session);
                  } catch (Exception e) {
            ...
            final Locale defaultLocale =
                determineDefaultLocale(LocaleContextHolder.getLocale(), infos);
            try {
              final File file = ImportUpdateService.getInstance().updateImport(
                  infos.getImportFile(), infos.getImportFileName(), infos.getType(),
                  new Version(infos.getOriginatingJahiaRelease()),
              ...
              try {
                ...
                final Resource finalLegacyMappingFilePath =
                    getLegacyMappingsInModules("map").get(legacyImportFilePath);
                final Resource finalLegacyDefinitionsFilePath =
                    getLegacyMappingsInModules("cnd").get(legacyDefinitionsFilePath);
                ...
                JCRObservationManager.doWithOperationType(null, JCRObservationManager.IMPORT,
                  ...
                  public Object doInJCR(JCRSessionWrapper jcrSession)
                    ...
                    try {
                      sitesService.addSite(jahiaUser, infos.getSiteTitle(),
                          infos.getSiteServername(), infos.getSiteKey(), "", defaultLocale,
                          finalTpl, null, "fileImport",
                          file == null ? null : new FileSystemResource(file),
                              infos.getImportFileName(), false, finalDoImportServerPermissions,
                              infos.getOriginatingJahiaRelease(), finalLegacyMappingFilePath,
                ...
                if (e.getCause() != null && (e.getCause() instanceof JahiaException
  ...
  private boolean processInternalImport(final JahiaUser jahiaUser, final MessageContext context, final JahiaSite system) {
    logger.info("Processing Import");
    boolean doImportServerPermissions = false;
    for (ImportInfo infos : importsInfos.values()) {
      if (infos.isSelected() && infos.getImportFileName().equals("serverPermissions.xml")) {
  ...
  private void doAsyncSiteImport(RequestContext flowReqContext, final JahiaUser jahiaUser, final MessageContext context) throws JahiaException {
    String siteKeyValue = null;
    final JahiaSite system = sitesService.getSiteByKey(JahiaSitesService.SYSTEM_SITE_KEY);
    
    Map<String, Object> paramMap = flowReqContext.getRequestParameters().asMap();
    for(Map.Entry<String, Object> paramEntry : paramMap.entrySet()) {
      System.out.println("[AFAC] - " + paramEntry.getKey() + " ->> " + paramEntry.getValue());
    ...
    ListenableFuture<Boolean> importTask =
        MoreExecutors.listeningDecorator(executorSrv).submit(new Callable<Boolean>() {
          ...
          @Override
          public Boolean call() throws Exception {
            /*final ThreadLocal<JahiaUser> expectedJahiaUser = new ThreadLocal<JahiaUser>();
            ...
            return processInternalImport(jahiaUser, context, system);
          }
        });

    logger.info("Import site submitted. User: {}, site key: {}", new Object[] {jahiaUser.getUsername(), siteKeyValue});
    String processKey = "import_jahia_site_" + siteKeyValue /*+ "_" + jahiaUser.getUsername()*/;
    AsyncImportSiteProcessManager.addSiteImportTask(processKey, importTask);
  }
